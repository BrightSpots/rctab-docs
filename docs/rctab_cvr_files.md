...
![A close up of text on a white backgroundDescription automatically generated][image50]
...
| ----- | :---- |
| **Column 1**  | Cast Vote Record ID number. Unique number, sequentially assigned. |
| **Column 2**  | Precinct Name that is associated with the cast vote record. |
| **Column 3**  | Ballot Style Name that is associated with the cast vote record. Applies to  both ballots by precinct and ballots by style elections. |
| **Column 4   thru N + 3** | N + 3 where N is the number of choices on the ballot. The content of the  cell in each of the columns will be the name of the candidate selected on  the ballot.   • If there is no selection for the choice, the content of the column will  show “undervote”.  • If there is more than one selection for the choice, the column will show  “overvote”.  • If the choice is a write-in the column will show “write-in”. |
...
For example, *1_c7db3cf2-c1a2-4773-971c-337ea2d198d7.xml*, or *50810795-95d4-4c0c-a6dc-0d5333e99c5a.xml*
