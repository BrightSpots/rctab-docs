...
1. Press F10 repeatedly until the BIOS menu appears.
1. Create a Workstation BIOS password for the workstation.
1. Navigating to ‘Security’ → ‘Create a BIOS Administrator Password’: Enter a Workstation BIOS password (at least 8 characters. Uppercase, Lowercase, 0-9, and symbol characters are all acceptable)
    1. Enter the password and press Enter.
    1. Enter the password again (to verify), and press Enter.
    1. **Please Note: There is no recovery to reset and re-enter the BIOS if the BIOS password is forgotten or lost.**
1. Insert a USB Stick with the Replicated Setup HpSetup.txt file.
1. Go to ‘Main’ → ‘Replicated Setup.’
1. Click ‘Restore current settings from USB device.’
1. Click ‘Browse for Configuration File’ and navigate to the HpSetup.txt on the USB 1 stick.
1. A prompt will appear that says ‘Number of settings successfully restored (239). Number of settings that were invalid (0).’ Hit ‘OK.’
1. Press F10 to save changes and exit.
1. The system will reboot multiple times. Confirm the settings change when prompted during the reboot processes.
...
1. Press F10 repeatedly until the BIOS menu appears.
1. Enter the Workstation BIOS Password.
1. Navigate to ‘Advanced \> Boot Options.’
1. Check the box to enable ‘USB Storage Boot.’
1. Press F10 and follow the prompts to save changes and exit.
1. Following OS deployment, follow the same process to disable USB Storage Boot.
...
1. Insert USB and restart the computer. When it restarts you should see a Windows OS installation screen. Follow the instructions.
    1. On the "Let's connect you to a network" screen select 'I don't have internet' button in bottom left then 'Continue with limited setup'
      ...
      ![][image11]
    1.
    1. When it asks Who’s going to use this PC? this will be the Windows Administrator Account. After providing an account name it will ask for a password.
      ![][image12]
      ![][image13]
    1. When asked, select the following settings to disable all Windows telemetry
      ![][image14]
...
1. Enter the Workstation BIOS Password.
1. Navigate to ‘Advanced \> Boot Options.’
1. Check the box to disable ‘USB Storage Boot.’
1. Press F10 and follow the prompts to save changes and exit.
...
1. Open Start.
1. Search for Control Panel and click the top result to open the app.
1. Click on System and Security.
1. Click on "BitLocker Drive Encryption."
...
10. Check the "Run BitLocker system check" option.
...
1. For each network connection right click on it: 4\) Select disable.
...
1. Search ‘Screen Saver’ and select ‘Change Screen Saver’

1. In the ‘Screen Saver’ dropdown select ‘None’

#### Disable Remote Access

1. Type *remote desktop settings* into the Windows search box.

1. Select *Remote Desktop Settings* to access the ‘Remote Desktop’ settings

1. Ensure ‘Enable Remote Desktop’ setting is set to **off**
...
1. Click on Windows Firewall.

1. In the left navigation pane click *Turn Windows Firewall On*.

#### Disable all network interfaces

1. Enter cmd in the windows search bar.

1. *Command Prompt* application shows up in search results.
...
1. For each network device listed enter the following command, replacing "Interface Name" with each of the names returned from step 5 (include the double-quotes):
