...
[**Section 23 - HashCode Instructions - Windows OS**](trusted_build_and_output_hash_verification.md)
...
1. Right- click on the `rcv.bat` file. Click ‘Run as Administrator.’ If a ‘Windows protected your PC’ window pops up click ‘More Info’ then click the ‘Run anyway’ button. Enter the administrator password
...
1. Change the current directory to the rcv folder created when you unzipped RCTab.
...
The following guide describes how to operate RCTab. See also [**Section 11 - L&A Testing**](l_and_a_testing.md) document for additional detail on system operations.
...
There are six tiebreak modes (described in technical detail in [**Section 25 - Configuration File Parameters**](configuration_file_parameters.md)). Tiebreak modes impact the validity/operation of one other option: random seed. Tiebreak mode is required for all winner election modes.
...
RCTab will not allow the output path to be configured to a Windows user account folder (by default anything under the path `c:\users\`). This requirement ensures read-only output. You **must** follow the instructions in [**Section 22 - Installation Instructions for Windows OS**](installation_instructions_for_windows_os.md) to set permissions on the output folder. Following these instructions ensures that the ‘RCTab’ user account cannot edit or delete RCTab summary output files or audit logs.
...
1. If any error messages appear in the log box, refer to 430 RCTab Operator Log Messages documentation and messages in the log box for how to resolve errors. If the error persists, restart RCTab software.
...
1. Tabulator log box will update with messages as Tabulation proceeds.
...
If necessary**,** instructions for verifying result file hashes can be found in [**Section 23 - Trusted Build & Output Hash Verification - Windows OS**](trusted_build_and_output_hash_verification.md).
...
If any errors arise in the use of RCTab, refer to [**Section 29 - RCTab Operator Log Messages**](rctab_operator_log_messages.md). Errors arising out of any hardware or software other than RCTab should refer to [**Section 09 - System Maintenance Manual**](system_maintenance_manual.md) and any relevant user and maintenance manuals.
